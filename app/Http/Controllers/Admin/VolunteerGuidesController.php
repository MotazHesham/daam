...
class VolunteerGuidesController extends Controller
{
    ...
    public function update_statuses(Request $request){ 
        $type = $request->type;
        $volunteerGuide = VolunteerGuide::findOrFail($request->id);
        $volunteerGuide->$type = $request->status; 
        $volunteerGuide->save();
        return 1;
    }
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = VolunteerGuide::query()->select(sprintf('%s.*', (new VolunteerGuide)->table));
            ...
            $table->editColumn('actions', function ($row) {
                $viewGate      = 'volunteer_guide_show';
                $editGate      = 'volunteer_guide_edit';
                $deleteGate    = 'volunteer_guide_delete';
                $crudRoutePart = 'volunteer-guides';
        ...
        return view('admin.volunteerGuides.index');
    }
