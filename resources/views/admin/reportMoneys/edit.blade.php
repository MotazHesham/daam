...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.report-moneys.update', [$reportMoney->id]) }}"
                enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label class="required">{{ trans('cruds.reportMoney.fields.part') }}</label>
                    <select class="form-control {{ $errors->has('part') ? 'is-invalid' : '' }}" name="part" id="part"
                        required>
                        <option value disabled {{ old('part', null) === null ? 'selected' : '' }}>
                            {{ trans('global.pleaseSelect') }}</option>
                        @foreach (App\Models\ReportMoney::PART_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('part', $reportMoney->part) === (string) $key ? 'selected' : '' }}>
                                {{ $label }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('part'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('year') ? 'is-invalid' : '' }}" type="number"
                        name="year" id="year" value="{{ old('year', $reportMoney->year) }}"  min="2000" max="2100"
                        required>
                    @if ($errors->has('year'))
                        <div class="invalid-feedback">
                            {{ $errors->first('year') }}
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text"
                        name="link" id="link" value="{{ old('link', $reportMoney->link) }}">
                    @if ($errors->has('link'))
                        <div class="invalid-feedback">
                            {{ $errors->first('link') }}
                        </div>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('published') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="published" id="published" value="1"
                            {{ $reportMoney->published || old('published', 0) === 1 ? 'checked' : '' }}>
                        <label class="form-check-label"
                            for="published">{{ trans('cruds.reportMoney.fields.published') }}</label>
                    </div>
